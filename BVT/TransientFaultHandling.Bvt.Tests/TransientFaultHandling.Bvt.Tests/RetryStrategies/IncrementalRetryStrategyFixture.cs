...
namespace Microsoft.Practices.EnterpriseLibrary.TransientFaultHandling.Bvt.Tests.RetryStrategies
{
    ...
    public class IncrementalRetryStrategyFixture
    {
        ...
        {
            var policy = retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Incremental, 5 retries");
        ...
        {
            var policy = retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Incremental, 5 retries, first retry is fast");
            ...
            catch (Exception)
            {
                ...
                for (int i = 2; i < 5; i++)
                {
                    var expectedIntervalInSeconds = i * TimeSpan.FromSeconds(1).Seconds; // Incremental Interval
        ...
        {
            var policy = retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Incremental, 5 retries, no fast retry");
        ...
        public void ExceptionIsThrownWhenFastFirstRetryIsEnabledAndNonTransientExceptionOccurs()
        {
            ...
            catch (Exception)
            {
                ...
                {
                    var expectedIntervalInSeconds = i * TimeSpan.FromSeconds(1).Seconds; // Incremental Interval
                    var actualIntervalInSeconds = ((TimeSpan)(elapsedTimes[i] - startTime)).Seconds;
                    Assert.AreEqual(expectedIntervalInSeconds, actualIntervalInSeconds, "i = " + i.ToString());
                }
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Incremental, 5 retries in Milliseconds");
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Incremental, 5 retries");
