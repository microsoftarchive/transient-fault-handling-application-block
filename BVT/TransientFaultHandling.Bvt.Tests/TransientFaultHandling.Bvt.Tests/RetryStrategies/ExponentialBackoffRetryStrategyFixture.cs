...
namespace Microsoft.Practices.EnterpriseLibrary.TransientFaultHandling.Bvt.Tests.RetryStrategies
{
    ...
    public class ExponentialBackoffRetryStrategyFixture
    {
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Exponential Backoff, 5 retries");
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Exponential Backoff, 5 retries, first retry is fast");
            ...
            catch (Exception)
            {
                ...
                for (int i = 2; i < 5; i++)
                {
                    var expectedMinIntervalInMilliseconds = 1000 + (int)((Math.Pow(2.0, i - 1) - 1.0) * (int)(200 * 0.8));
                    var expectedMaxIntervalInMilliseconds = 1000 + (int)((Math.Pow(2.0, i - 1) - 1.0) * (int)(200 * 1.2));
                    var actualIntervalInMilliseconds = elapsedTimes[i].TotalMilliseconds - elapsedTimes[i - 1].TotalMilliseconds;
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Exponential Backoff, 5 retries, no fast retry");
        ...
        {
            var policy = this.retryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Exponential Backoff, 5 retries in Milliseconds");
